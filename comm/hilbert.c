...
static bitmask_t getIEEEBits(unsigned nDims, unsigned ignoreMe, /* ignored */
                     ...
                     bitmask_t bits, bitmask_t index, BitReader getBits)
{
    ...
    while (y--)
    {
        ...
        if (diff)
        {
            ...
            bitmask_t digit =
                rotateRight(bits ^ reflection, rotation, nDims);
            for (d = 1; d < nDims; d *= 2)
            {
                index ^= index >> d;
                digit ^= (digit >> d) & ~smear;
...
unsigned
hilbert_ieee_box_vtx(unsigned nDims, int findMin, double *c1, double *c2)
{
    unsigned rotation, max;
    bitmask_t bits, index;
    if (getIEEESignBits(nDims, c1) != getIEEESignBits(nDims, c2))
...
hilbert_box_pt_work(unsigned nDims, unsigned nBytes, unsigned nBits,
                    ...
                    bitmask_t bits,
                    bitmask_t index,
                    BitReader getBits, BitWriter propogateBits)
{
    bitmask_t const one = 1;
...
unsigned
hilbert_ieee_box_pt(unsigned nDims, int findMin, double *c1, double *c2)
{
    unsigned rotation, max;
    bitmask_t bits, index;
    ...
    if (c1Signs != c2Signs)
    {
        ...
        hilbert_box_pt_work(nDims, 8, 64, findMin,
                            IEEErepBits - 1, IEEErepBits, (char *) c1,
                            (char *) c2, rotation, bits, index,
    ...
    while (y-- > 0)
    {
               ...
               dimensions don't matter. */
        {
                   ...
                   "here" part. If there is no "there" there, skip it */
            {
                ...
                p_firstSeparator =
                    rotateLeft(p_firstSeparator, rotation, nDims);
                p_cornerdiff =
                    cornerdiff & ~(p_separator ^ p_firstSeparator);
                p_y = y;
    ...
    for (;;)
    {
        ...
        for (r = 0; r < nPrints; ++r)
        {
            ...
            for (r1 = 0; r1 < r; ++r1)
            {
                ...
                ans =
                    hilbert_cmp(nDims, sizeof(coord[0]), nBits, coord,
                                coord1);
                if (ans != 1)
                {
            ...
            hilbert_i2c(nDims, nBits, r1, coord1);
            if (hilbert_cmp(nDims, sizeof(coord[0]), nBits, coord, coord1)
                != 0)
...
void sfc(const unsigned *nDims, const unsigned *nBits, bitmask_t * coord)
{
    unsigned nPrints, r;
    nPrints = 1ULL << ((*nDims) * (*nBits));
    ...
    for (r = 0; r < nPrints; ++r)
    {
        hilbert_i2c(*nDims, *nBits, r, &coord[r * *nDims]);
    }
}
...
int main()
{
    unsigned nDims = 2;
    unsigned nBits = 1;
    unsigned nPrints = 1ULL << (nDims * nBits);
    unsigned r, i;
    bitmask_t coord[nPrints * nDims];
    ...
    for (r = 0; r < nPrints; ++r)
    {
        ...
        {
            coord[r * nDims + i] = 0;
            printf(" %d", (unsigned) coord[r * nDims + i]);
        }
        printf("\n");
    }

    printf("Call ... \n");
    sfc(&nDims, &nBits, coord);
...
{
    return hilbert_cmp(g_nDims, sizeof(unsigned), 8 * sizeof(unsigned),
                       c1p, c2p);
}
...
int main()
{
    ...
    for (i = 0; i < 10000; ++i)
    {
        ...
        for (k = 0; k < (1 << nDims); k += (1 << nDims) - 1)
        {
            ...
            {
                mismatch =
                    (eltk[j] != ((k == 0) ? cornerlo : cornerhi)[j]);
            }
            assert(!mismatch);
        }
        free((char *) array);
...
int main()
{
    ...
    printf("Enter other point (%d coordinates, strictly greater): ",
           nDims);
    for (k = 0; k < nDims; ++k)
    {
        unsigned diff;
    ...
    array =
        (array_t *) malloc(maxDim * sizeof(unsigned) *
                           (outvolume - involume));
    if (array == 0)
    {
...
int main()
{
    ...
    stat =
        hilbert_nextinbox(nDims, nBytes, 8 * nBytes, findPrev, c1, c2, pt);
